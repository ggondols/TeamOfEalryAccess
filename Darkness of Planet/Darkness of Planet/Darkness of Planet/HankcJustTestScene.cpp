...
static DWORD WINAPI ThFunc1(LPVOID lpParam)
{
	...
	HankcJustTestScene* temp = (HankcJustTestScene*)lpParam;
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 10; j++)
		{
			...
			pSkinnedMesh->SetCallbackfunction(bind(&HankcJustTestScene::CallbackOn, temp, (i + 1) * 10 + j));
...
HankcJustTestScene::HankcJustTestScene()
	: m_pMap(NULL)
	, m_pNode(NULL)
	, m_bThread(false)
	, m_pAstar(NULL)
...
HRESULT HankcJustTestScene::Setup()
{
	//////////정현
...
void HankcJustTestScene::Release()
{
	m_pGrid->Release();
...
void HankcJustTestScene::Update()
{
...
void HankcJustTestScene::CallbackOn(int number)
{
	//////0 캐릭터   10~29 적   
...
bool HankcJustTestScene::CollisionCheck(TeicEnemy * A, TeicEnemy * B)
{
...
void HankcJustTestScene::TotalPushCheck2()
{
	for (int i = 0; i < m_vecEnemy.size(); i++)
	{
...
void HankcJustTestScene::ChangeGridInfo()
{
	if (m_bThread)
	{
		for (int i = 0; i < m_vecEnemy.size(); i++)
...
void HankcJustTestScene::TargetOn()
{
...
bool HankcJustTestScene::CheckStraight(TeicEnemy *A)
{
...
float HankcJustTestScene::EnemyEnemyDistance(TeicEnemy * A, TeicEnemy * B)
{
	float length;
...
float HankcJustTestScene::EnemyPlayerDistance(TeicEnemy *ene)
{
	float length;
...
void HankcJustTestScene::Render()
{
...
bool HankcJustTestScene::TotalCollisionCheck()
{
	bool check;
	check = false;
	for (int i = 0; i < m_vecEnemy.size(); i++)
...
bool HankcJustTestScene::CheckSlot()
{
	bool temp;
	int count = -1;
...
bool HankcJustTestScene::ChangeCheckPoint()
{
...
void HankcJustTestScene::AngleChange(TeicEnemy * A)
{
...
void HankcJustTestScene::Push2(TeicEnemy * A, TeicEnemy * B)
{
...
void HankcJustTestScene::WayUpdate()
{
