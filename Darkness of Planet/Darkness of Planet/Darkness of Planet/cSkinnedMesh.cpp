...
cSkinnedMesh::cSkinnedMesh(char* szFolder, char* szFilename)
	...
	, m_fRadious(0.5f)
{
	cSkinnedMesh* pSkinnedMesh =  SKINMANAGER->GetSkinnedMesh(szFolder, szFilename);
...
void cSkinnedMesh::UpdateAndRender(D3DXMATRIX * pmatParent)
{
	if (m_pAnimController)
	{
		m_pAnimController->AdvanceTime(TIMEMANAGER->getElapsedTime(), NULL);
	...
	if (m_pRootFrame)
	{
		Update(m_pRootFrame, (D3DXMATRIXA16*)pmatParent);
